...
A script to recursively scan a directory for audio files (mp3, m4b, m4a, wav, flac, ogg, aac) and split them into equally sized chunks if they exceed a certain duration threshold.
...
- `AUDIO_MAX_DURATION_SZ`: Maximum duration threshold (in seconds). Only files exceeding this length will be split.
- `AUDIO_CUT_CHUNK_SZ`: The duration of each split chunk (in seconds).
